...
btn.MouseButton1Click:Connect(function()
        ...
        -- 2. On lance la vérification dans une thread séparée 
        -- pour ne pas bloquer l'UI, mais on garde le Kick ici.
        task.spawn(function()
            local valid = isKeyValid(input.Text)
            
            if not valid then
                -- Le Kick est la seule chose qui compte ici
                player:Kick("Invalid key! Access denied.")
            else
                -- Si valide, on nettoie
                gui:Destroy()
                startScript()
            end
        end)
